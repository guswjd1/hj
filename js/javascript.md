...
```
모두 true 일때만 결과값 true
true && true → true
true % false → false

모두 false 일때만 결과값 false
true || false → true
...
- 조건문, 분기문
- 조건식(비교식)의 결과값(true/false)에 따라 실행 코드를 분기하는 명령문
...
- 집합체 형태의 데이터
- 비슷한 형태의 데이터끼리 하나의 집합체 형태로 묶어서 사용
  - 비슷한 형태
    - 같은 타입의 데이터 : 배열
